...
```{r}
colnames(pData(phenoData(gse25462[[1]])))
...
Exprs() generates a matrix containing expression level for each probe.
...
arrayQualityMetrics(expressionset = gse25462[[1]],
    ...
    intgroup = c("characteristics_ch1.3"))
...
The above indicates the degree of variation from the mean for each sample. The samples with higher deviation from the mean indicate these samples have different expression in some way. 

The row median calculation indicates the median gene expression for each gene. The boxplot gives a general sense of the expression variation per sample by indicating 
...
ggplot(dataGG, aes(PC1, PC2)) +
  ...
  coord_fixed(ratio = sd_ratio)
#  scale_shape_manual(values = c(4,15)) + 
#  scale_color_manual(values = c("darkorange2", "dodgerblue4", "green", "orange"))
...
phenotype_names <- unique(gse_norm$characteristics_ch1.3)
disease_names <- unique(gse_norm$insulin_category)

annotation_for_heatmap <- 
  data.frame(Phenotype = phenotype_names,  Disease = disease_names)

row.names(annotation_for_heatmap) <- row.names(pData(gse25462[[1]]))
