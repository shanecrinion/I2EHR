...
RLE_class_patient <- data.frame(patient_array = rownames(pData(subset.patient)), 
                                disease_cat = pData(subset.patient)$`diabetes:ch1`)
...
subset.control_gse <- subset.control$geo_accession[1:3]
subset.control <- subset.control[,c(subset.control$`geo_accession` == subset.control_gse[1:3])]


View(pData(genomic_data[[1])
###### END CONTROL
...
RLE_class_control <- data.frame(patient_array = rownames(pData(subset.control)), 
                                disease_cat = pData(subset.control)$`diabetes:ch1`)
